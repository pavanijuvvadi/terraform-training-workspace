...
terraform{
backend "s3" {
    bucket = "terrraform-pyh-training-state"
    region = "ap-southeast-1" # emight want to use ap-southeast-1
    key    = "pyh/exercise-01/terraform.tfstate" # MAKE SURE TO USE A DIFFERENT KEY THAN YOUR TEAM MEMBERS!!!
    encrypt = true
    dynamodb_table = "terrraform-pyh-training-lock"
}
}
